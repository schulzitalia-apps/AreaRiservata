...
function TrendArrow({ direction, color }: { direction: "up" | "down"; color: string }) {
  return (
    <svg
      viewBox="0 0 32 32"
      className={cn("h-5 w-5 shrink-0 sm:h-7 sm:w-7 md:h-8 md:w-8", direction === "down" && "rotate-180")}
...
export function BilancioTachimetro({
  ...
  const pctLabel = data.relativePctLabel && data.relativePctLabel.trim().length > 0 ? data.relativePctLabel : `${Math.round(rawPct * 100)}%`;
  ...
  const activeAngle = isProfit ? pctToArcDeg(arcPct01, RIGHT_START, RIGHT_END) : pctToArcDeg(arcPct01, LEFT_START, LEFT_END);
  ...
  return (
    <div className={cn("relative aspect-square w-full", className)}>
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 20%, ${hexToRgba(goodTo, 0.18)} 0%, transparent 60%), radial-gradient(circle at 30% 86%, ${hexToRgba(badTo, 0.14)} 0%, transparent 62%), radial-gradient(circle at 78% 86%, ${hexToRgba(goodFrom, 0.16)} 0%, transparent 62%)`,
        }}
      />
      ...
      <svg viewBox={`0 0 ${V} ${V}`} className="h-full w-full">
        <defs>
          ...
          <linearGradient id={ids.gradGoodDim} gradientUnits="userSpaceOnUse" x1={rightStartPt.x} y1={rightStartPt.y} x2={rightEndPt.x} y2={rightEndPt.y}>
            <stop offset="0%" stopColor={goodFrom} stopOpacity="0.18" />
            <stop offset="100%" stopColor={goodTo} stopOpacity="0.18" />
          </linearGradient>

          <linearGradient id={ids.gradBadDim} gradientUnits="userSpaceOnUse" x1={leftStartPt.x} y1={leftStartPt.y} x2={leftEndPt.x} y2={leftEndPt.y}>
            <stop offset="0%" stopColor={badFrom} stopOpacity="0.18" />
            <stop offset="100%" stopColor={badTo} stopOpacity="0.18" />
          </linearGradient>

          <linearGradient id={ids.gradGoodActive} gradientUnits="userSpaceOnUse" x1={rightStartPt.x} y1={rightStartPt.y} x2={rightEndPt.x} y2={rightEndPt.y}>
          ...
          <linearGradient id={ids.gradBadActive} gradientUnits="userSpaceOnUse" x1={leftStartPt.x} y1={leftStartPt.y} x2={leftEndPt.x} y2={leftEndPt.y}>
        ...
        <path d={rightTrack} stroke={`url(#${ids.gradGoodDim})`} strokeWidth={stroke} strokeLinecap="round" fill="none" />
        <path d={leftTrack} stroke={`url(#${ids.gradBadDim})`} strokeWidth={stroke} strokeLinecap="round" fill="none" />
        ...
        <path
          ...
          style={{ transition: "opacity 320ms ease, stroke-dasharray 950ms cubic-bezier(0.16, 1, 0.3, 1)" }}
        />
        <path
          d={leftTrack}
        ...
        >
          <circle cx={cx + r} cy={cy} r={20} fill={isProfit ? hexToRgba(goodTo, 0.22) : hexToRgba(badTo, 0.22)} />
          <circle cx={cx + r} cy={cy} r={8} fill="#FFFFFF" opacity={0.92} />
        </g>
      </svg>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-3 text-center">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="text-4xl font-black leading-none text-white sm:text-5xl md:text-7xl">{pctLabel}</div>
        </div>

        <div
          className="mt-2 text-xl font-extrabold sm:mt-3 sm:text-2xl md:text-4xl"
          style={{ color: profitColor, textShadow: `0 0 16px ${hexToRgba(profitColor, 0.25)}` }}
        >
          {signedEuro(profit)}
        </div>

        <div className="mt-1 max-w-[13rem] text-[11px] font-semibold text-white/60 sm:mt-2 sm:max-w-[16rem] sm:text-sm md:text-base">
          {periodLabel}
        </div>
      </div>
    </div>
