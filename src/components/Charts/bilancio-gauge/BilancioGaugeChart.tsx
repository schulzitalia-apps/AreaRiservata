...
function TrendArrow({ direction, color }: { direction: "up" | "down"; color: string }) {
  return (
    <svg
      viewBox="0 0 32 32"
      className={cn("h-9 w-9 shrink-0", direction === "down" && "rotate-180")}
      style={{
        filter: `drop-shadow(0 0 10px ${hexToRgba(color, 0.45)})`,
      }}
      aria-hidden="true"
    >
      <path
...
export function BilancioGaugeChart({
                                     periodLabel,
                                     data,
                                     className,
  ...
  const ricavi = Math.max(0, Math.round(Number(data.ricavi || 0)));
  const spese = Math.max(0, Math.round(Number(data.spese || 0)));
  ...
  const pctLabel = (data.relativePctLabel && data.relativePctLabel.trim().length > 0)
    ? data.relativePctLabel
    : `${Math.round(rawPct * 100)}%`;

  // per arco clamp a 0..1 (se >100% saturi l’arco ma mantieni label)
  ...
  const ids = useMemo(() => {
    return {
      ...
      // gradient “progressivo” (sfumato verso il knob)
  ...
  return (
    <div
      className={cn(
        "rounded-3xl border border-emerald-400/80 bg-black/70 p-6",
        "shadow-[0_0_0_1px_rgba(16,185,129,0.12),0_40px_120px_rgba(0,0,0,0.55)]",
        className,
      )}
    >
      <div className="text-sm font-extrabold text-white">% Margine profitto</div>

      <div className="mt-4 grid gap-4 lg:grid-cols-12 lg:items-center">
        {/* LEFT: SPESE */}
        <div className="lg:col-span-2">
          <div className="text-xs font-semibold" style={{ color: hexToRgba(badTo, 0.70) }}>
            Spese
          </div>
          <div className="mt-1 text-2xl font-extrabold text-white">{euro(spese)}</div>
        </div>
            ...
            <div className="relative aspect-square w-full">
              <svg viewBox={`0 0 ${V} ${V}`} className="h-full w-full">
                <defs>
                  ...
                  {/* ACTIVE gradients: sfumato “mano a mano” verso il knob */}
                  <linearGradient
                ...
                <path
                  ...
                  style={{
                    transition:
                      "opacity 320ms ease, stroke-dasharray 950ms cubic-bezier(0.16, 1, 0.3, 1)",
                  }}
                />
                <path
                  d={leftTrack}
                ...
                <g
                  ...
                  style={{
                    ...
                    transition:
                      "opacity 220ms ease, transform 950ms cubic-bezier(0.16, 1, 0.3, 1)",
                  }}
                >
                  {/* halo */}
                  <circle
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                ...
                <div
                  ...
                  style={{
                    color: profitColor,
                    textShadow: `0 0 16px ${hexToRgba(profitColor, 0.25)}`,
                  }}
                >
                  {signedEuro(profit)}
                </div>

                <div className="mt-2 text-sm font-semibold text-white/60 md:text-base">
                  {periodLabel}
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 text-right">
          <div className="text-xs font-semibold" style={{ color: hexToRgba(goodFrom, 0.80) }}>
            Ricavi
          </div>
          <div className="mt-1 text-2xl font-extrabold text-white">{euro(ricavi)}</div>
        </div>
      </div>
    </div>
  );
