...
function coerceDate(m: any, mi: number): Date {
  const d = m?.date;
  if (d instanceof Date && Number.isFinite(d.getTime())) return d;

  // supporto eventuale ISO string
  if (typeof d === "string") {
...
function splitAmount(total: number, seed: number): number[] {
  ...
  const r2 = 0.20 + noise01(seed + 29) * 0.20; // 0.20..0.40
...
export function buildTransactions(
  monthly: Array<{ date?: Date; byCategory: Record<keyof PctSet, number> }>,
  opts: {
    suppliers: string[];
  ...
  monthly.forEach((m, mi) => {
    const monthDate = coerceDate(m, mi);

    cats.forEach((k, ki) => {
      const totalRaw = (m as any)?.byCategory?.[k];
      const total = Number.isFinite(totalRaw) ? Number(totalRaw) : 0;
      if (total <= 0) return;
      ...
      parts.forEach((amount, c) => {
        ...
        out.push({
          id: `${mi}-${String(k)}-${c}-${amount}`, // stabile
          title: t,
          supplier,
