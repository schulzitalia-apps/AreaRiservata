...
function splitPrevCurrentMonths(apiData: SpeseAnalyticsResponse, timeKey: TimeKey): { prev: any[]; current: any[] } {
  const months = Array.isArray(apiData?.months) ? apiData.months : [];
...
export function buildTotalsLikeFromApi(args: { apiData: SpeseAnalyticsResponse; timeKey: TimeKey }) {
...
export function buildMonthlyFromApi(args: { apiData: SpeseAnalyticsResponse; timeKey: TimeKey }) {
  const { current } = splitPrevCurrentMonths(args.apiData, args.timeKey);
...
function normalizeTopExpenseItem(variantId: string, item: any) {
  const effectiveDate = safeStr(item?.effectiveDate ?? item?.dataFatturazione ?? item?.dataSpesa ?? "");

  return {
    id: safeStr(item?.id ?? item?._id ?? `${variantId}-${Math.random().toString(16).slice(2)}`),
    title: safeStr(item?.titolo ?? (item as any)?.title ?? "Spesa"),
    supplier: item?.fornitore ? safeStr(item.fornitore) : null,
    amount: Math.round(safeNum(item?.totaleLordo ?? item?.lordo ?? item?.amount ?? 0)),
    category: variantId,
    date: effectiveDate,
...
export function buildTop5FromApi(args: {
  ...
  if (args.catKey === "all") {
    ...
    for (const variantId of Object.keys(topBucket)) {
      ...
      for (const item of items) {
        if (matchesCurrentPeriod(item)) merged.push(normalizeTopExpenseItem(variantId, item));
      }
    }
  ...
  return items
    ...
    .map((item) => normalizeTopExpenseItem(variantId, item))
    .sort((a, b) => safeNum(b.amount) - safeNum(a.amount))
    .slice(0, 5);
}

export function buildUpcomingIpotizzatoFromApi(args: { apiData?: SpeseAnalyticsResponse }) {
  if (!args.apiData) return [];
  ...
  for (const variantId of Object.keys(bucket)) {
    ...
    for (const item of items) {
      ...
      out.push({
        ...
        dateLabel: relativeDateLabel(item?.effectiveDate ?? item?.dataFatturazione ?? item?.dataSpesa ?? ""),
      });
    }
  }
