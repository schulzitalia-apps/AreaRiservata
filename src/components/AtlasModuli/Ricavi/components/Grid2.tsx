...
type Props = {
  ...
  memos: any[];
  memoOpen: boolean;
  setMemoOpen: (value: boolean | ((previous: boolean) => boolean)) => void;
  memoTitle: string;
  setMemoTitle: (value: string) => void;
  memoDate: string;
  setMemoDate: (value: string) => void;
  memoAmount: number;
  setMemoAmount: (value: number) => void;
  onAddMemo: () => void;
};
    ...
    <div className="grid gap-6 lg:grid-cols-12 lg:items-stretch">
      <div className="lg:col-span-3">
        <Card className="h-full">
          <CardHeader
            title="Top 5 ricavi piu' alti"
            right={<span className="rounded-full border border-stroke bg-white/60 px-3 py-1 text-xs font-extrabold text-gray-700 dark:border-dark-3 dark:bg-gray-dark/40 dark:text-white/80">{props.catLabel}</span>}
          />
        ...
        <Card className="h-full">
          <div className="px-5 pt-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-xs font-semibold text-gray-500 dark:text-dark-6">Ricavi per mese nel periodo selezionato</div>
                <div className="mt-1 text-lg font-extrabold text-dark dark:text-white">
                  Mese per mese <span className="ml-2 text-sm font-semibold text-gray-500 dark:text-dark-6">({props.currentPeriodLabel})</span>
                </div>
              </div>
            ...
            <div className="mt-4">
              <CategoryTabsDyn value={props.catKey} onChange={props.setCatKey} items={props.categoryTabItems} />
            </div>
          </div>
          ...
          <div className="px-4 pb-5 pt-3">
            <ApexBarChart
              ...
              options={{
                tooltip: {
                  y: {
                    formatter: (value: number) => euro(Number(value)),
                  },
                },
              }}
            />
            ...
            <div className="mt-3 text-sm font-semibold text-gray-600 dark:text-dark-6">
              {props.catKey === "all" ? (
                <>
                  Baseline: <span className="font-extrabold text-dark dark:text-white">Totale</span>
                </>
              ) : (
                <>
                  Baseline: <span className="font-extrabold text-dark dark:text-white">Totale</span>
                  ...
                  Riempimento:{" "}
                  <span className="font-extrabold text-dark dark:text-white" style={{ color: props.catColor }}>
                    {props.catLabel}
                  </span>
                </>
              )}
        ...
        <Card className="h-full">
          <CardHeader
            ...
            right={
              <button
                type="button"
                onClick={() => props.setMemoOpen((value: boolean) => !value)}
              ...
              <div className="mb-4 rounded-2xl border border-stroke bg-white p-3 dark:border-dark-3 dark:bg-gray-dark">
                <div className="grid gap-2">
                  <input
                    ...
                    onChange={(event) => props.setMemoTitle(event.target.value)}
                    placeholder="Titolo"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(event) => props.setMemoDate(event.target.value)}
                      className="w-full rounded-lg border border-stroke bg-white px-3 py-2 text-sm font-semibold text-dark outline-none focus:border-primary/40 dark:border-dark-3 dark:bg-gray-dark dark:text-white dark:focus:border-primary/50"
                    />
                    <input
                      type="number"
                      value={props.memoAmount || ""}
                      onChange={(event) => props.setMemoAmount(Number(event.target.value))}
                      placeholder="Importo"
                      className="w-full rounded-lg border border-stroke bg-white px-3 py-2 text-sm font-semibold text-dark outline-none focus:border-primary/40 dark:border-dark-3 dark:bg-gray-dark dark:text-white dark:focus:border-primary/50"
                    />
                  </div>
