...
import { MenuPayload } from "./useContextMenu";
...
function contextParts(payload: MenuPayload) {
  ...
  const time = payload.kind === "hour" ? `${pad2(payload.hour)}:00` : null;
...
type Props = {
  ...
  /** elimina evento (solo se payload.kind === "event") */
...
export default function CalendarContextMenu({
  ...
  const handleViewEvent = () => {
    if (!isEvent || typeof window === "undefined") return;
    ...
    >
      ...
      >
        ...
        <button
          ...
          role="menuitem"
        >
          {isDayView ? "📅 Torna alla vista mese" : "🕒 Vai alla vista giorno"}
        </button>
      </ContextMenuPanel>
    </div>
  );
