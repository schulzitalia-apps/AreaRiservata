    ...
    <Dropdown isOpen={isOpen} setIsOpen={setIsOpen}>
      <DropdownTrigger className="rounded align-middle outline-none ring-primary ring-offset-2 focus-visible:ring-1 dark:ring-offset-gray-dark">
        ...
        <figure className="flex items-center gap-3">
          <AvatarImg alt={`Avatar of ${displayName}`} className="shrink-0" />
          <figcaption className="flex min-w-0 items-center gap-1 font-medium text-dark dark:text-dark-6 max-[1024px]:sr-only">
      ...
      <DropdownContent
        className="min-[230px]:min-w-[17.5rem] max-w-[calc(100vw-1rem)] border border-stroke bg-white shadow-md dark:border-dark-3 dark:bg-black"
        align="end"
      >
        <h2 className="sr-only">User information</h2>

        <figure className="flex items-center gap-2.5 px-5 py-3.5">
          <AvatarImg alt={`Avatar for ${displayName}`} className="shrink-0" />
          <figcaption className="min-w-0 flex-1 space-y-1 text-base font-medium">
            <div
              className="mb-2 truncate leading-none text-dark dark:text-white"
              title={displayName}
            >
              {displayName}
            </div>
            <div
              className="truncate leading-none text-gray-6"
              title={displayEmail}
            >
              {displayEmail}
            </div>
